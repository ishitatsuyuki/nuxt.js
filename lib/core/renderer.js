...
export default class Renderer {
  constructor(nuxt) {
    ...
    this.app = new Koa()
    // For legacy (connect) error middleware compatibility
    this.connectErrorMiddlewares = []
  ...
  useMiddleware(m) {
    ...
    if (!m.koa) {
      if (handler.length === 4) {
        ...
        return this.connectErrorMiddlewares.push(handler)
      } else {
        handler = koaConnect(handler)
      }
    }

    // Use middleware
    this.app.use(koaMount(path, handler))
  }
  ...
  async setupMiddleware() {
    // Connect error handlers compat
    this.app.use(async (ctx, next) => {
      try {
        await next()
      } catch (originalErr) {
        let err = originalErr
        for (const handler of this.connectErrorMiddlewares) {
          try {
            await pify(handler)(err, ctx.req, ctx.res)
          } catch (newErr) {
            err = newErr
          }
        }
    ...
    if (!this.options.dev && this.options.render.gzip) {
      this.useMiddleware({
        handler: koaCompress(this.options.render.gzip),
        koa: true
      })
    }
      ...
      this.useMiddleware({
        handler: async (ctx, next) => {
          if (this.webpackMiddleware) {
            return (await this.webpackMiddleware)(ctx, next)
          }
          return next()
        },
        koa: true
    ...
    this.useMiddleware({
      prefix: this.options.render.static.prefix,
      handler: koaStatic(
